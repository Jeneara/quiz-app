...
function showResult() {
    ...
    let winnerStyle = winners.length !== 1
...
function renderQuestion() {
    ...
    <div class="space-y-2">
      ${q.answers.map(a => `<button onclick="answer('${a.results}')" class="block w-full bg-gray-100 hover:bg-gray-200 p-4 rounded">${a.text}</button>`).join("")}
    </div>
  `;
}
